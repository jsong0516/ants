...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            # assert self.ant.can_contain(insect) or insect.contain_ant(self.ant), 'Two ants in {0}'.format(self)
        else:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            #self.ant.ant.place = self.ant.place

            if type(insect) is QueenAnt and insect.is_first_queen:
                pass
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            while i < len(bees):
                ...
                if temp_len - len(bees) == 0: # Check if the length of list is changed. If it does, no need to increase index. By this, no copy list is required.
                    i += 1
...
class QueenPlace:
    ...
    def bees(self):
        "*** YOUR CODE HERE ***"
        temp = []
        if self and self.colony_queen:
            temp += self.colony_queen.bees
        if self.ant_queen:
            temp += self.ant_queen.bees
        return temp
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        if QueenAnt.is_first_queen:
            self.is_first_queen = True
            QueenAnt.is_first_queen = False
        else:
            self.armor = 0 # I think this is not correct
            
    def action(self, colony):
        """A queen ant throws a leaf, but also doubles the damage of ants
        in her tunnel.  Impostor queens do only one thing: die."""
        
        def double_damage(curr_ant):
            if not curr_ant.damage_doubled:
                curr_ant.damage_doubled = True
                curr_ant.damage *= 2
            if curr_ant.container and curr_ant.ant and not curr_ant.ant.damage_doubled:
                curr_ant.ant.damage_doubled = True
                curr_ant.ant.damage *= 2
        
        if self.armor >= 0: 
            
            colony.queen = QueenPlace(colony.queen, self.place)
            
            ThrowerAnt.action(self, colony) # Throw

            # Exit
            curr_place = self.place
            while curr_place.exit:
                curr_ant = curr_place.exit.ant
                if curr_ant:
                    double_damage(curr_ant)
                curr_place = curr_place.exit

            # Entrance
            curr_place = self.place
            while curr_place.entrance:
                curr_ant = curr_place.entrance.ant
                if curr_ant:
                    double_damage(curr_ant)
